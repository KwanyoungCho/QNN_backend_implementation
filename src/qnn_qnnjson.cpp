...
// bool parse_qnn_sdk_json(const std::string& json_path,
...
//   // iterate graphs
//   size_t pos = 0;
//   while (find_next(s, pos, "\"graphName\"")) {
//     if (!find_next(s, pos, ":")) return false;
//     std::string gname; if (!parse_string(s, pos, gname)) return false;
//     QnnJsonGraphDesc g; g.graph_name = gname;

//     // inputs
//     size_t in_pos = pos;
//     if (!find_next(s, in_pos, "\"graphInputs\"")) return false;
//     if (!find_next(s, in_pos, "[")) return false;
//     while (true) {
//       size_t obj = s.find("{\n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t", in_pos);
//       size_t end = s.find("}", in_pos);
//       // simple termination condition
//       if (s.find("\"graphOutputs\"", in_pos) != std::string::npos && s.find("\"graphOutputs\"", in_pos) < end) break;
//       // parse one tensor entry by scanning fixed keys
//       size_t tp = in_pos;
//       if (!find_next(s, tp, "\"name\"")) break;
//       if (!find_next(s, tp, ":")) break;
//       QnnJsonTensorDesc td;
//       if (!parse_string(s, tp, td.name)) break;
//       // dtype
//       size_t dp = in_pos;
//       if (!find_next(s, dp, "\"dataType\"")) break;
//       if (!find_next(s, dp, ":")) break;
//       if (!parse_string(s, dp, td.data_type)) break;
//       // dims
//       size_t mp = in_pos;
//       if (!find_next(s, mp, "\"dimensions\"")) break;
//       if (!find_next(s, mp, ":")) break;
//       if (!parse_dims(s, mp, td.dims)) break;
//       // compute nbytes
//       uint64_t numel = 1; for (auto v : td.dims) numel *= v;
//       td.nbytes = numel * elem_size_from_dtype(td.data_type);
//       g.inputs.push_back(std::move(td));
//       // advance to next element or outputs
//       size_t next = s.find("},", in_pos);
//       size_t outputs_key = s.find("\"graphOutputs\"", in_pos);
//       if (outputs_key != std::string::npos && (next == std::string::npos || outputs_key < next)) {
//         in_pos = outputs_key; break;
//       }
...
//     // outputs
//     size_t out_pos = in_pos;
//     if (!find_next(s, out_pos, "\"graphOutputs\"")) return false;
//     if (!find_next(s, out_pos, "[")) return false;
//     while (true) {
//       size_t tp = out_pos;
//       if (!find_next(s, tp, "\"name\"")) break;
//       if (!find_next(s, tp, ":")) break;
...
//       uint64_t numel = 1; for (auto v : td.dims) numel *= v;
//       td.nbytes = numel * elem_size_from_dtype(td.data_type);
//       g.outputs.push_back(std::move(td));
//       size_t next = s.find("},", out_pos);
//       size_t graphs_key = s.find("\"graphs\"", out_pos);
//       if (graphs_key != std::string::npos && (next == std::string::npos || graphs_key < next)) {
//         out_pos = graphs_key; break;
//       }
//       if (next == std::string::npos) break;
//       out_pos = next + 2;
//     }
...
// 엄격 모드: 그래프 오브젝트 경계를 먼저 찾고, 그 내부에서만 배열을 파싱한다.
